...
struct AppPrimaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .shadow(
                color: AppTheme.Colors.shadow.opacity(configuration.isPressed ? 0.35 : 0.5),
                radius: AppTheme.Layout.softShadowRadius,
                x: 0,
                y: configuration.isPressed ? 1 : AppTheme.Layout.cartoonShadowOffset
            )
    }
}
...
struct AppSecondaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .shadow(
                color: AppTheme.Colors.shadow.opacity(configuration.isPressed ? 0.28 : 0.42),
                radius: AppTheme.Layout.softShadowRadius,
                x: 0,
                y: configuration.isPressed ? 1 : AppTheme.Layout.cartoonShadowOffset
            )
