...
4. Assistant session follow-up memory path:
   1. `backend/crates/shared/src/assistant_memory.rs`
   2. `backend/crates/enclave-runtime/src/http/assistant/memory.rs`
   3. `backend/crates/enclave-runtime/src/http/assistant/session_state.rs`
   4. `backend/crates/shared/src/repos/assistant_encrypted_sessions.rs`
...
5. Privacy delete purges encrypted assistant session rows in `purge_user_operational_data`.
...
3. Additional content-blind boundary guards live in:
   1. `backend/crates/shared/src/enclave/tests/boundary_guards.rs`
